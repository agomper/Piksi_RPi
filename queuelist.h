...
void QueueList<T>::push (const T i) {
  ...
  if (tail == NULL)
    printf("QUEUE: insufficient memory to create a new node.");
...
T QueueList<T>::pop () {
  // check if the queue is empty.
  if (isEmpty ())
    printf("QUEUE: can't pop item from queue: queue is empty.");
...
T QueueList<T>::peek () const {
  // check if the queue is empty.
  if (isEmpty ())
    printf("QUEUE: can't peek item from queue: queue is empty.");
